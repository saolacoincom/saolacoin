...
func checkTrieContents(t *testing.T, db Database, root []byte, content map[string][]byte) {
	...
	trie, err := New(meter.BytesToBytes32(root), db)
	if err != nil {
		t.Fatalf("failed to create trie at %x: %v", root, err)
	}
	if err := checkTrieConsistency(db, meter.BytesToBytes32(root)); err != nil {
...
func checkTrieConsistency(db Database, root meter.Bytes32) error {
...
func TestEmptyTrieSync(t *testing.T) {
	emptyA, _ := New(meter.Bytes32{}, nil)
	emptyB, _ := New(emptyRoot, nil)

	for i, trie := range []*Trie{emptyA, emptyB} {
		db := ethdb.NewMemDatabase()
		if req := NewTrieSync(meter.BytesToBytes32(trie.Root()), db, nil).Missing(1); len(req) != 0 {
...
func testIterativeTrieSync(t *testing.T, batch int) {
	...
	sched := NewTrieSync(meter.BytesToBytes32(srcTrie.Root()), dstDb, nil)

	queue := append([]meter.Bytes32{}, sched.Missing(batch)...)
	for len(queue) > 0 {
...
func TestIterativeDelayedTrieSync(t *testing.T) {
	...
	queue := append([]meter.Bytes32{}, sched.Missing(10000)...)
	for len(queue) > 0 {
...
func testIterativeRandomTrieSync(t *testing.T, batch int) {
	...
	queue := make(map[meter.Bytes32]struct{})
...
func TestDuplicateAvoidanceTrieSync(t *testing.T) {
	...
	queue := append([]meter.Bytes32{}, sched.Missing(0)...)
	requested := make(map[meter.Bytes32]struct{})
...
func TestIncompleteTrieSync(t *testing.T) {
	...
	added := []meter.Bytes32{}
	queue := append([]meter.Bytes32{}, sched.Missing(1)...)
	for len(queue) > 0 {
